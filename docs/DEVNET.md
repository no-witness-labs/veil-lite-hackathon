...
- The `veil-lite-0.5.0.dar` package deployed/vetted on the validator for a new test environment.
...
3. Allocates five role parties plus a distinct issuer:
   `veilLiteLender`, `veilLiteBorrower`, `veilLiteRegulator`, `veilLiteValuer`, `veilLiteOutsider`, `veilLiteDemoIssuer` (with the chosen suffix).
4. Grants `CanActAs` for those parties to `VEIL_LEDGER_USER_ID` (default: `6`).
5. Seeds the canonical holdings with issuer and owner authority: lender 100 cash, borrower 105 cash + 150 collateral + a separate 50-unit reserve. No real asset backing is claimed.
...
- Do not upload `veil-0.1.0.dar`; that package name collides with an existing
  DevNet package. Use `veil-lite-0.5.0.dar` for this branch.
