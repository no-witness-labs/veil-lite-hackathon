...
| 0:58–1:08 | Click `Outsider`; open the `Raw ledger` tab; expand `Raw ledger view`. | “An outsider is not a stakeholder.” Point to `Not a stakeholder.`, the `0 contracts` badge, and the literal `[]` response. This is a party-scoped Canton query, not a hidden UI element. |
...
| 2:34–3:00 | Click `Lender`; open the `Raw ledger` tab and expand `Raw ledger view`; then open the `Activity` tab. | “The proof is on-ledger: show opaque contract IDs, the `offset`, each transaction’s `tx` update ID, `sync` synchronizer ID, and created/archived IDs. The UI selects the configured issuer for operational deal views, while this raw response remains the unabridged party query.” Stop at 3:00. |
